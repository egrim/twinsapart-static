...
e?a.vars.pauseText:a.vars.playText),a.pausePlay.bind(a.eventType,function(c){c.preventDefault();b(this).hasClass("pause")?(a.pause(),a.manualPause=!0):(a.resume(),a.manualPause=!1)}));if("ontouchstart"in document.documentElement){var h,i,j,k,d,n,l=!1;a.each(function(){"ontouchstart"in document.documentElement&&this.addEventListener("touchstart",s,!1)});var s=function(c){a.animating?c.preventDefault():1==c.touches.length&&(a.pause(),k=a.vertical?a.height():a.width(),n=Number(new Date),j=a.vertical?
(a.currentSlide+a.cloneOffset)*a.height():(a.currentSlide+a.cloneOffset)*a.width(),h=a.vertical?c.touches[0].pageY:c.touches[0].pageX,i=a.vertical?c.touches[0].pageX:c.touches[0].pageY,a.setTransition(0),this.addEventListener("touchmove",p,!1),this.addEventListener("touchend",q,!1))},p=function(c){d=a.vertical?h-c.touches[0].pageY:h-c.touches[0].pageX;l=a.vertical?Math.abs(d)<Math.abs(c.touches[0].pageX-i):Math.abs(d)<Math.abs(c.touches[0].pageY-i);l||(c.preventDefault(),"slide"==a.vars.animation&&
a.transitions&&(a.vars.animationLoop||(d/=0==a.currentSlide&&0>d||a.currentSlide==a.count-1&&0<d?Math.abs(d)/k+2:1),a.args[a.prop]=a.vertical?"translate3d(0,"+(-j-d)+"px,0)":"translate3d("+(-j-d)+"px,0,0)",a.container.css(a.args)))},q=function(){a.animating=!1;if(a.animatingTo==a.currentSlide&&!l&&null!=d){var c=0<d?a.getTarget("next"):a.getTarget("prev");a.canAdvance(c)&&550>Number(new Date)-n&&20<Math.abs(d)||Math.abs(d)>k/2?a.flexAnimate(c,a.vars.pauseOnAction):a.flexAnimate(a.currentSlide,a.vars.pauseOnAction)}this.removeEventListener("touchmove",
p,!1);this.removeEventListener("touchend",q,!1);j=d=i=h=null}}"slide"==a.vars.animation.toLowerCase()&&b(window).resize(function(){a.animating||(a.vertical?(a.height(a.slides.filter(":first").height()),a.args[a.prop]=-1*(a.currentSlide+a.cloneOffset)*a.slides.filter(":first").height()+"px"):(a.newSlides.width(a.width()),a.args[a.prop]=-1*(a.currentSlide+a.cloneOffset)*a.width()+"px"),a.transitions&&(a.setTransition(0),a.args[a.prop]=a.vertical?"translate3d(0,"+a.args[a.prop]+",0)":"translate3d("+
